  ...
  wrap.innerHTML = `
    ...
    <div class="section-block" style="max-width:520px;margin:0 auto">
      <div style="display:flex;flex-direction:column;gap:22px;padding:8px 0">
        ...
        <div class="form-group">
          <label style="font-weight:700;font-size:15px;display:flex;align-items:center;gap:8px">
            <span style="display:inline-flex;align-items:center;justify-content:center;background:var(--primary);color:#fff;border-radius:50%;width:24px;height:24px;font-size:13px">3</span>
            MONTANT À PAYER
          </label>
          <input id="cabineMontant" type="number" min="100" step="50" placeholder="Ex : 500"
            oninput="updateCabineMontant()"
            style="width:100%;padding:13px 14px;border:2px solid var(--border);border-radius:var(--radius);font-size:16px;margin-top:10px;box-sizing:border-box;letter-spacing:1px" />
          <div id="cabineMontantInfo" style="font-size:13px;color:var(--muted);margin-top:6px">
            💡 Frais de service de 1% ajoutés au montant saisi.
          </div>
        </div>
        ...
        <div class="form-group">
          <label style="font-weight:700;font-size:15px;margin-bottom:12px;display:block">
            <span style="display:inline-flex;align-items:center;justify-content:center;background:var(--primary);color:#fff;border-radius:50%;width:24px;height:24px;font-size:13px;margin-right:6px">4</span>
            TYPE DE SERVICE
          </label>
          <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px">
...
function updateCabineMontant() {
  const raw = Number((document.getElementById("cabineMontant")?.value || "").trim());
  const info = document.getElementById("cabineMontantInfo");
  if (!info) return;
  if (!raw || raw < 100) {
    info.style.color = "var(--muted)";
    info.innerHTML = "💡 Frais de service de 1% ajoutés au montant saisi.";
    return;
  }
  const total = Math.round(raw * 1.01);
  const fees = total - raw;
  info.style.color = "#2e7d32";
  info.innerHTML = `✓ Montant saisi : <strong>${raw} FCFA</strong> · Frais 1% : <strong>+${fees}</strong> · <strong style="color:#e65100">Total à payer : ${total} FCFA</strong>`;
}
...
function cabinePayer() {
  ...
  const montantRaw = Number((document.getElementById("cabineMontant")?.value || "").trim());
  if (!montantRaw || montantRaw < 100) return toast("Entrez un montant valide (min 100 FCFA)", "red");
  ...
  const montantFinal = Math.round(montantRaw * 1.01);
  const fees = montantFinal - montantRaw;
  const waMsg = encodeURIComponent(
    `🎟 Commande Cabine en Ligne\n` +
    `${opInfo.ico || "📱"} Opérateur : ${op}\n` +
    `☎️ Numéro : ${num}\n` +
    `⚙️ Service : ${svc}\n` +
    `💰 Montant : ${montantRaw} FCFA (+${fees} frais = ${montantFinal} FCFA)\n\n` +
    `Merci de traiter cette commande.`
  );
  const smsText = `Cabine : ${svc} ${op} → ${num} — ${montantFinal} FCFA`;

  // 1. Ouvrir le lien de paiement immédiatement
  const sep = CABINE_PAYMENT_LINK.includes("?") ? "&" : "?";
  window.open(`${CABINE_PAYMENT_LINK}${sep}amount=${montantFinal}`, "_blank");
  ...
  modalHTML(`
    ...
    <div style="background:#e8f5e9;border-left:4px solid #2e7d32;border-radius:0 var(--radius) var(--radius) 0;padding:12px 14px;margin-bottom:14px;font-size:13px;line-height:1.8">
      ...
      ⚙️ Service : <strong>${svc}</strong><br>
      💰 Montant : <strong>${montantRaw} FCFA</strong> + 1% frais = <strong style="color:#e65100">${montantFinal} FCFA</strong>
    </div>
...
async function loadFlashSection() {
  ...
  // Ordre aléatoire à chaque chargement
  for (let i = _flashAll.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [_flashAll[i], _flashAll[j]] = [_flashAll[j], _flashAll[i]];
  }
  renderFlashSection();
}
...
function renderCityBanner() {
  ...
  if (!el) return;
  // Bannière masquée uniquement lorsque l'onglet actif est "concours" ou "emploi".
  const hideForTab = _page2Cat === "concours" || _page2Cat === "concours-ci" || _page2Cat === "emploi";
  if (hideForTab) {
    el.innerHTML = "";
    el.style.display = "none";
    return;
  }
  const label = SELECTED_CITY
    ? `📍 Ville : <strong>${SELECTED_CITY}</strong>`
    : `🌍 Vous consultez les annonces de : <strong>Toutes les villes</strong>`;
  el.style.display = "";
  el.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;background:#fff8f2;border:1px solid #ffd9b8;border-radius:10px;padding:10px 14px;margin:10px 0;font-size:13px;color:#5d3a1f">
      <span>${label}</span>
      <button onclick="showCityModal()" style="background:var(--primary);color:#fff;border:none;padding:6px 12px;border-radius:16px;font-size:12px;font-weight:600;cursor:pointer">Changer</button>
    </div>`;
}
